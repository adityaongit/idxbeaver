...
export function ContentSection({ title, children, id }: { title: string; children: ReactNode; id?: string }) {
  return (
    <section id={id} className="mt-12">
      ...
      <div className="space-y-4 text-[15px] leading-[1.72] text-[var(--color-ink-dim)] [&_a]:underline [&_a]:decoration-[var(--color-hair-2)] [&_a]:underline-offset-4 [&_a:hover]:text-[var(--color-ink)] [&_code]:mono [&_code]:rounded [&_code]:bg-[var(--color-hair)] [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:text-[13px] [&_strong]:text-[var(--color-ink)] [&_ul]:ml-5 [&_ul]:list-disc [&_ul]:space-y-1.5 [&_ol]:ml-5 [&_ol]:list-decimal [&_ol]:space-y-1.5 [&_pre]:overflow-x-auto [&_pre]:rounded-lg [&_pre]:border [&_pre]:border-[var(--color-hair)] [&_pre]:bg-[var(--color-bg-2)] [&_pre]:p-4 [&_pre]:text-[13px] [&_pre]:leading-[1.6] [&_h3]:mt-8 [&_h3]:text-[18px] [&_h3]:font-medium [&_h3]:text-[var(--color-ink)]">
        {children}
      </div>
    </section>
  );
